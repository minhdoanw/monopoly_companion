...
const Monopoly = (() => {
  ...
  // ─── CARDS DEFINITION ────────────────────────────────────────────────────────
  const CHANCE_CARDS = [
    { title: "Advance to GO", desc: "Advance to GO (Collect 200)", action: (s, p) => movePlayerTo(s, p, 0, true) },
    { title: "Advance to Illinois / Vịnh Hạ Long", desc: "Advance to Space 24 (If you pass GO, collect 200)", action: (s, p) => movePlayerTo(s, p, 24, true) },
    { title: "Advance to St. Charles / Cố đô Huế", desc: "Advance to Space 11 (If you pass GO, collect 200)", action: (s, p) => movePlayerTo(s, p, 11, true) },
    {
      title: "Advance to nearest Utility",
      desc: "Advance to nearest Utility. If owned, pay owner 10x dice roll. If unowned, you may buy it.",
      action: (s, p) => {
    ...
    },
    {
      ...
      desc: "Advance to nearest Railroad. If owned, pay owner double rent. If unowned, you may buy it.",
      action: (s, p) => {
        let pos = p.position;
        let target = 5;
    ...
    { title: "Bank Pays Dividend", desc: "Bank pays you dividend of 50", action: (s, p) => payFromBank(s, p, 50, "CARD_EFFECT") },
    { title: "Get Out of Jail Free", desc: "Get Out of Jail Free card. This card may be kept until needed.", action: (s, p) => { p.getOutOfJailCards++; } },
    {
      title: "Go Back 3 Spaces",
      desc: "Retreat 3 spaces.",
      action: (s, p) => {
        let target = (p.position - 3 + 40) % 40;
        movePlayerTo(s, p, target, false);
      }
    },
    { title: "Go to Jail", desc: "Go directly to Jail. Do not pass GO, do not collect 200.", action: (s, p) => sendToJail(s, p) },
    {
    ...
    { title: "Speeding Fine", desc: "Speeding fine 15", action: (s, p) => payToBank(s, p, 15, "CARD_EFFECT") },
    { title: "Take a trip to Reading Railroad / Ga Sài Gòn", desc: "Advance to Space 5 (If you pass GO, collect 200)", action: (s, p) => movePlayerTo(s, p, 5, true) },
    { title: "Advance to Boardwalk / Nguyễn Huệ", desc: "Advance to Space 39", action: (s, p) => movePlayerTo(s, p, 39, true) },
    {
    ...
    { title: "Building Loan Matures", desc: "Your building loan matures. Collect 150.", action: (s, p) => payFromBank(s, p, 150, "CARD_EFFECT") }
  ];

  const CHEST_CARDS = [
    { title: "Advance to GO", desc: "Advance to GO (Collect 200)", action: (s, p) => movePlayerTo(s, p, 0, true) },
    { title: "Bank Error in your favor", desc: "Bank error in your favor. Collect 200.", action: (s, p) => payFromBank(s, p, 200, "CARD_EFFECT") },
    { title: "Doctor's Fees", desc: "Doctor's fees. Pay 50.", action: (s, p) => payToBank(s, p, 50, "CARD_EFFECT") },
    { title: "Stock Sale", desc: "From sale of stock you get 50.", action: (s, p) => payFromBank(s, p, 50, "CARD_EFFECT") },
    { title: "Get Out of Jail Free", desc: "Get Out of Jail Free card. This card may be kept until needed.", action: (s, p) => { p.getOutOfJailCards++; } },
    { title: "Go directly to Jail", desc: "Go directly to Jail. Do not pass GO, do not collect 200.", action: (s, p) => sendToJail(s, p) },
    { title: "Holiday Fund Matures", desc: "Holiday fund matures. Receive 100.", action: (s, p) => payFromBank(s, p, 100, "CARD_EFFECT") },
    { title: "Income Tax Refund", desc: "Income tax refund. Collect 20.", action: (s, p) => payFromBank(s, p, 20, "CARD_EFFECT") },
    {
      title: "Birthday Party",
      desc: "It is your birthday. Collect 10 from every player.",
      action: (s, p) => {
        s.players.forEach(other => {
          if (other.id !== p.id && !other.isBankrupt) {
    ...
    { title: "Life Insurance Matures", desc: "Life insurance matures. Collect 100.", action: (s, p) => payFromBank(s, p, 100, "CARD_EFFECT") },
    { title: "Hospital Fees", desc: "Pay hospital fees of 100.", action: (s, p) => payToBank(s, p, 100, "CARD_EFFECT") },
    { title: "School Fees", desc: "Pay school fees of 50.", action: (s, p) => payToBank(s, p, 50, "CARD_EFFECT") },
    { title: "Consultancy Fee", desc: "Receive 25 consultancy fee.", action: (s, p) => payFromBank(s, p, 25, "CARD_EFFECT") },
    {
      title: "Street Repairs",
      desc: "You are assessed for street repairs. For each house pay 40. For each hotel pay 115.",
      action: (s, p) => {
        let { houses, hotels } = countPlayerBuildings(s, p.id);
    ...
    { title: "Beauty Contest", desc: "You have won second prize in a beauty contest. Collect 10.", action: (s, p) => payFromBank(s, p, 10, "CARD_EFFECT") },
    { title: "Inheritance", desc: "You inherit 100.", action: (s, p) => payFromBank(s, p, 100, "CARD_EFFECT") }
  ];
